...
Этот Telegram-бот предоставляет интерфейс для запуска DAG в Apache Airflow с параметрами, задаваемыми пользователем на естественном языке.

Принцип работы:

Понимание запроса: Для парсинга пользовательских запросов используется LLM (GigaChat). Модель получает полный контекст проекта: маппинг DAG'ов, их переменные, связанные таблицы, а также бизнес-описание таблиц и их взаимосвязей. Это позволяет точно понимать даже разговорные формулировки и продуктивный "сленг".

Исполнение: После обработки запроса LLM извлекает необходимые параметры и передает управление готовым Python-методам. Нейросеть не генерирует код, а лишь выступает интеллектуальным роутером, что исключает неконтролируемые действия и гарантирует выполнение процессов по заранее прописанным сценариям.

Таким образом, система заменяет собой сложные условные конструкции (if-else) на гибкое понимание естественного языка, без потери надежности и предсказуемости выполнения.
